    ...
    try {
        ...
        if ($res_mesin_reset && $res_mesin_reset->num_rows > 0) {
            while($rm = $res_mesin_reset->fetch_assoc()) {
                ...
                $res_p = $conn->query("SELECT prodCount FROM log_quality WHERE mcID = '$mcID_r' AND id <= $max_q ORDER BY id ASC");
                $prod_r = 0;
                if ($res_p && $res_p->num_rows > 0) {
                    ...
                    while ($rp = $res_p->fetch_assoc()) {
                        ...
                        $delta = $curr_p - $prev_p;
                        if ($delta > 0) $prod_r += $delta;
                        else if ($delta < 0) $prod_r += $curr_p;
                        $prev_p = $curr_p;
                    }
                }
                ...
                // if ($a_r > 100) $a_r = 100; // Removed cap
                // if ($p_r > 100) $p_r = 100; // Removed cap
                if ($q_r > 100) $q_r = 100;
                if ($q_r < 0) $q_r = 0;
        ...
        // Delete ONLY the migrated records (Safe Reset)
        if ($max_q > 0) {
...
if ($result && $result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        $sql_prod_logs = "SELECT prodCount FROM log_quality WHERE mcID = '$mcID' AND timestamp >= '$waktu_mulai' AND timestamp <= '$waktu_selesai' ORDER BY timestamp ASC, id ASC";
        $res_prod_logs = $conn->query($sql_prod_logs);
        
        $prodCount = 0;
        $prev_prodCount = $offset_produksi;
        if ($res_prod_logs && $res_prod_logs->num_rows > 0) {
            while($p_row = $res_prod_logs->fetch_assoc()) {
                $curr_p = (int)$p_row['prodCount'];
                $d_p = $curr_p - $prev_prodCount;
                if ($d_p > 0) $prodCount += $d_p;
                else if ($d_p < 0) $prodCount += $curr_p;
                $prev_prodCount = $curr_p;
            }
        }
