...
void setup() {
  ...
  // Tambahkan pelindung Auto-Recovery: Jika jalur I2C terkena gangguan elektromagnetik (EMI) dari antena RFID,
  // Mega akan otomatis me-reset hardware I2C-nya. Timeout 25ms (25000 us).
  ...
  // --- Inisialisasi Keypad ---
  keypad.setDebounceTime(20); // Turunkan dari default 50ms menjadi 20ms agar merespon sentuhan cepat
  ...
  if (!versiondata) {
    ...
    // PENTING: Batasi jumlah retry PN532 agar tidak memblokir / me-lag loop utama
    // 0x10 (16 kali) cukup untuk membangunkan kartu yang sedang 'tertidur' tanpa bikin lag
...
void loop() {
  ...
  // HANYA polling RFID saat di layar LOGIN_NIK, MAIN_SCREEN, INFO_SCREEN, atau SKILL_CHECK.
  // Jika di layar INPUT_PROCESS atau INPUT_NG, RFID dimatikan sesaat agar Keypad TIDAK LAG saat diketik!
  if (rfidHardwareFound && (currentState == LOGIN_NIK || currentState == MAIN_SCREEN || currentState == INFO_SCREEN || currentState == SKILL_CHECK) && (currentMillis - lastRfidCheck > 500)) {
  ...
  // Timeout diset ke 50ms. Ini adalah waktu minimal agar komunikasi Serial2 dengan kartu bisa selesai 100%.
  ...
  while (Serial2.available()) Serial2.read();
    ...
    if (success) {
      ...
      // Auto-Recovery: Jika gagal 10x berturut-turut (5 detik), reinit PN532
      // Threshold 10 agar kartu tidak terlalu sering kehilangan power RF
      if (rfidConsecutiveFails >= 10) {
        ...
        // PENTING: Wajib pasang kembali batasan retry setelah Reinit! 
        // Jika tidak dipasang, PN532 akan kembali ke settingan bawaan (Infinite Retry) yang menyebabkan LAG!
        nfc.setPassiveActivationRetries(0x10); 
  ...
  // FITUR BARU: Pause Timer Auto-Logout saat sedang mengetik input atau menunggu balasan server
  if (currentState == INPUT_PROCESS || currentState == INPUT_NG_QTY || currentState == INPUT_NG_CODE || currentState == SETUP_MODE ||
      currentState == WAIT_NIK_REPLY || currentState == WAIT_PROCESS_REPLY || currentState == WAIT_SETUP_REPLY) {
      lastCardDetectTime = currentMillis; // Terus perbarui timer seolah-olah kartu ditahan
  }

  if (current_uid != "") {
      if (currentMillis - lastCardDetectTime > 4000) {
  ...
  switch (currentState) {
    ...
    case SKILL_CHECK:
      if (key == 'C') {
        digitalWrite(interlockPin, HIGH); // Pastikan terkunci
        op_NIK = "";
        current_uid = "";
        op_Level = 0;
        changeState(LOGIN_NIK);
        lcd.clear(); lcd.setCursor(0, 0); lcd.print(F("Dibatalkan..."));
        delay(1000);
        forceUpdate = true;
      }
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (topicStr == "SMMS/Reply/Setup" && currentState == WAIT_SETUP_REPLY) {
    if (status == "registered") {
      ...
      writeStringToEEPROM(EEPROM_DISPLAY_ID_ADDR, display_idMesin);
...
void readNGButtons() {
  ...
  if (ngSubState == LOW) {
    ...
    if (!ngSubPressed) {
      ...
      // Reset counter Arduino Nano via I2C dengan mengirim Magic Byte 0xAA
      Wire.beginTransmission(8);  // Alamat Arduino Nano (Slave)
      Wire.write(0xAA);           // Kirim perintah reset
...
unsigned int readCounterFromNano() {
  ...
  if (bytesRead != 2) {
    Serial.print(F("[DEBUG] I2C Error! Menerima "));
    Serial.print(bytesRead);
    Serial.println(F(" byte dari Nano. Mereset I2C Bus..."));
    // SELF-HEALING: Radiasi antena RFID yang kuat saat memindai sering membuat sinyal I2C Mega (LCD/Nano) crash.
    // Jika itu terjadi, kita reset paksa hardware I2C Mega agar bisa digunakan lagi di detik berikutnya.
    Wire.begin(); 
  }
...
String readStringFromEEPROM(int addr) {
  ...
  while (len < 31) {
    ...
    if (Wire.available()) {
      char c = Wire.read();
      // Berhenti membaca jika ketemu Null Terminator (\0) ATAU memori kosong (0xFF)
