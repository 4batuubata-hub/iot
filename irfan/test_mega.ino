...
void setup() {
  ...
  // Pelindung ini dinonaktifkan sementara karena bisa mengganggu Nano Slave
  ...
  // --- Scan I2C Bus (Diagnostik koneksi) ---
  // Dihapus karena ping 0-byte (Wire.endTransmission()) bisa menyebabkan Arduino Nano (Slave) crash/hang.
  Serial.println(F(""));
  Serial.println(F("=== I2C BUS SCAN SKIPPED ==="));
  Serial.println(F("===================="));
  Serial.println(F(""));
...
void loop() {
  ...
  // --- IDENTIFIKASI STATE INPUT UNTUK PAUSE RFID ---
  // Saat user sedang mengetik di keypad atau menunggu konfirmasi, RFID dipause agar tidak ada lag.
  bool isInputState = (currentState == INPUT_PROCESS || currentState == INPUT_NG_QTY || 
                       currentState == INPUT_NG_CODE || currentState == SETUP_MODE ||
                       currentState == CONFIRM_PROCESS || currentState == WAIT_PROCESS_REPLY ||
                       currentState == WAIT_SETUP_REPLY || currentState == INFO_SCREEN);

  // Jika sedang input, kita refresh timer kartu agar tidak otomatis logout (interlock)
  if (isInputState && current_uid != "") {
      lastCardDetectTime = currentMillis;
  }

  // --- LOGIKA CONTINUOUS RFID HOLD SYSTEM ---
  // Polling RFID di SEMUA state kecuali BOOT dan state input
  if (rfidHardwareFound && currentState != BOOT && !isInputState && (currentMillis - lastRfidCheck > 2000)) {
  ...
  if (current_uid != "") {
      // Toleransi 6 detik karena RFID sekarang di-polling setiap 2 detik
      if (currentMillis - lastCardDetectTime > 6000) {
  ...
  // Diubah menjadi continuous polling agar respons instan seperti kode lama
  cycleCountReceived = readCounterFromNano();
  
  // Perhitungan produksi: offset + counter Nano (sama dengan code_project_lama)
...
void readNGButtons() {
  ...
  if (ngSubState == LOW) {
    ...
    if (!ngSubPressed) {
      ...
      // Reset counter Arduino Nano via I2C (sama seperti code_project_lama)
      Wire.beginTransmission(8);  // Alamat Arduino Nano (Slave)
      Wire.endTransmission();     // Kirim sinyal reset
