...
void loop() {
  ...
  if (rfidHardwareFound && (currentState == LOGIN_NIK || currentState == MAIN_SCREEN || currentState == INFO_SCREEN) && (currentMillis - lastRfidCheck > 1000)) {
  ...
  // Timeout diturunkan ke 20ms agar tidak me-lag (membekukan) pembacaan keypad / tombol
  ...
  while (Serial2.available()) Serial2.read();
    ...
    if (success) {
      ...
      // Auto-Recovery: Jika gagal 4x berturut-turut (2 detik), reinit PN532
      // Membangunkan kartu sebelum terkena batas timeout logout (3 detik)
      if (rfidConsecutiveFails >= 4) {
  ...
  // FITUR BARU: Pause Timer Auto-Logout HANYA saat sedang mengetik input
