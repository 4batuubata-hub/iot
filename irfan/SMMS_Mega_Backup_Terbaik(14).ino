...
// -------------------- MAIN SCREEN --------------------
    ...
    case MAIN_SCREEN:
      ...
      else if (key == 'C') { // Tombol C = Logout Manual
        op_NIK = "";
        op_Name = "";
        op_Level = 0;
        digitalWrite(interlockPin, HIGH); // Kunci mesin
        lcd.clear(); lcd.setCursor(0, 0); lcd.print(F("Logout Berhasil!"));
        delay(1500);
...
// =====================================================================
  ...
  // =====================================================================
  //          BACA COUNTER DARI NANO (TERUS-MENERUS)
  // Harus dipanggil setiap iterasi loop agar angka yang masuk ke Nano 
  // "tertangkap" oleh Mega SEBELUM glitch RFID mereset Nano secara paksa.
  // =====================================================================
  unsigned int currentNano = readCounterFromNano();
  
  // AUTO-ACCUMULATE LOGIC (ANTI-GLITCH)
  // Jika Nano tiba-tiba mereset dirinya ke 0 (karena EMI RFID menyebabkan glitch I2C),
  // Mega mendeteksi penurunan nilai dan menambahkannya ke cycleCountSaved.
  if (currentNano < cycleCountReceived) {
    cycleCountSaved += cycleCountReceived;
  }
  cycleCountReceived = currentNano;
  
  unsigned int cycleCount = cycleCountSaved + cycleCountReceived;
  prodCount = cycleCount * cavity;
  OKCount = prodCount - NGCount;
  
  // DEBUG: Tampilkan ke Serial Monitor (dibatasi 1 detik agar tidak spam Serial)
