...
$sql_logs = "SELECT lq.timestamp, lq.prodCount, m.part_number, m.part_name, m.proses_name, m.proses_description, m.ct_pcs, m.ct_jam, IFNULL(m.cavity, 1) as cavity
             FROM log_quality lq
...
$true_total_prod = 0;
$prev_prodCount = (int)$offset_produksi;
...
if ($res_logs && $res_logs->num_rows > 0) {
    while($row = $res_logs->fetch_assoc()) {
        ...
        if ($curr_prodCount < 0) continue; // Abaikan glitch negatif
        ...
        if ($delta > 0 && $delta <= $max_delta) {
            ...
            // ESP32 Ter-Reset dari 0
            $qty_added = $curr_prodCount; 
            $prev_prodCount = $curr_prodCount;
        } else if ($delta == 0) {
            // Idle
        } else {
            // Noise spike (> max_delta): abaikan
            continue;
        }
