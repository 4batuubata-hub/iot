...
$filterTanggal = isset($_GET['tanggal']) ? $conn->real_escape_string($_GET['tanggal']) : '';
...
// Default ke tanggal terbaru hanya jika pertama kali load halaman tanpa parameter apapun
if (!isset($_GET['tanggal']) && !isset($_GET['mesin']) && !isset($_GET['shift'])) {
    $filterTanggal = $max_tanggal;
}

// Validasi format tanggal jika bukan ALL / kosong
if (!empty($filterTanggal) && $filterTanggal !== 'ALL' && !preg_match('/^\d{4}-\d{2}-\d{2}$/', $filterTanggal)) {
    $filterTanggal = 'ALL';
}

// Query Data History Summary dengan Filter Gabungan
$where_clauses = [];
if (!empty($filterTanggal) && $filterTanggal !== 'ALL') {
    $where_clauses[] = "hs.tanggal = '$filterTanggal'";
}
if ($filterMesin !== 'ALL') {
    $where_clauses[] = "(hs.mcID = '$filterMesin' OR mm.id_mesin = '$filterMesin' OR mm.mcID = '$filterMesin')";
}
if ($filterShift !== 'ALL') {
    $where_clauses[] = "hs.shift = '$filterShift'";
}

$where_sql = count($where_clauses) > 0 ? "WHERE " . implode(" AND ", $where_clauses) : "";
$query_history = "SELECT hs.*, mm.nama_mesin as master_nama, mm.id_mesin as master_id 
    ...
    <div class="header">
        ...
        <form method="GET" id="filterForm" class="filter-container">
            ...
            <div style="display: flex; gap: 6px; align-items: center;">
                <input type="date" name="tanggal" id="inputTanggal" class="filter-select" 
                       value="<?= ($filterTanggal !== 'ALL' && !empty($filterTanggal)) ? htmlspecialchars($filterTanggal) : '' ?>" 
                       min="<?= $min_tanggal ?>" max="<?= $max_tanggal ?>"
                       onchange="document.getElementById('filterForm').submit()">
                <?php if ($filterTanggal !== 'ALL' && !empty($filterTanggal)): ?>
                    <button type="button" class="filter-select" style="padding: 10px 12px; cursor: pointer;" title="Tampilkan Semua Tanggal" onclick="document.getElementById('inputTanggal').value=''; document.getElementById('filterForm').submit();">📅 Semua</button>
                <?php endif; ?>
            </div>
    ...
    <div class="grid-container">
        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $last_part = !empty($row['part_name']) ? $row['part_name'] : '';
                
                if (empty($last_part) && $wm && $ws) {
                    $sql_lp = "SELECT mc.part_name FROM history_quality lq LEFT JOIN master_ct mc ON lq.kode_proses = mc.kode WHERE lq.mcID = '$mcID_card' AND lq.timestamp >= '$wm' AND lq.timestamp <= '$ws' ORDER BY lq.timestamp DESC LIMIT 1";
                    $res_lp = $conn->query($sql_lp);
                    if ($res_lp && $res_lp->num_rows > 0) {
                        $last_part = $res_lp->fetch_assoc()['part_name'];
                    }
                }
                if (empty($last_part)) $last_part = '-';
                
                $display_nama = $row['master_nama'] ?: $row['nama_mesin'];
                $display_id = $row['master_id'] ?: $row['mcID'];
                
                ?>
                <div class="card <?= $oeeClass ?>" onclick="window.location.href='detail.php?id=<?= $row['id'] ?>&mcID=<?= urlencode($row['mcID']) ?>&tanggal=<?= $row['tanggal'] ?>&shift=<?= urlencode($row['shift']) ?>'">
