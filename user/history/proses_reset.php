...
try {
    ...
    if ($max_q > 0) $conn->query("INSERT IGNORE INTO history_quality (timestamp, mcID, kode_proses, op_NIK, mcStatus, mcInfo, prodCount, NGCount) SELECT timestamp, mcID, kode_proses, op_NIK, mcStatus, mcInfo, prodCount, NGCount FROM log_quality WHERE id <= $max_q");
    ...
    if ($res_mesin && $res_mesin->num_rows > 0) {
        while($row = $res_mesin->fetch_assoc()) {
            ...
            $res_p = $conn->query("SELECT prodCount FROM log_quality WHERE mcID = '$mcID' AND id <= $max_q ORDER BY id ASC");
            $prod = 0;
