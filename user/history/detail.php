...
// Alokasi memori & waktu eksekusi yang cukup untuk membaca arsip berukuran besar
...
// Jika diberikan ID arsip summary, ambil mcID, tanggal, dan shift secara otomatis
if ($id > 0) {
    $res_by_id = $conn->query("SELECT * FROM history_summary WHERE id = $id LIMIT 1");
    if ($res_by_id && $res_by_id->num_rows > 0) {
        $row_by_id = $res_by_id->fetch_assoc();
        if (empty($mcID)) $mcID = $row_by_id['mcID'];
        if (empty($tanggal)) $tanggal = $row_by_id['tanggal'];
        if (empty($filterShift)) $filterShift = $row_by_id['shift'];
    }
}
...
$sql_summary = "SELECT hs.*, mm.nama_mesin, mm.id_mesin as string_mcID 
                FROM history_summary hs 
                LEFT JOIN master_mesin mm ON (hs.mcID = mm.id_mesin OR hs.mcID = mm.mcID) 
                WHERE ";
if ($id > 0) {
    $sql_summary .= "hs.id = $id LIMIT 1";
} else {
    $sql_summary .= "hs.mcID = '$mcID' AND hs.tanggal = '$tanggal'";
...
if (empty($filterShift) || $filterShift == 'REKAP HARIAN' || $filterShift == 'ALL') {
    $filter_waktu_hq = "hq.timestamp >= '$tanggal 00:00:00' AND hq.timestamp <= '$tanggal 23:59:59'";
    $filter_waktu_hn = "hn.timestamp >= '$tanggal 00:00:00' AND hn.timestamp <= '$tanggal 23:59:59'";
    $filter_waktu_hd = "hd.timestamp >= '$tanggal 00:00:00' AND hd.timestamp <= '$tanggal 23:59:59'";
} else {
...
// Master CT mapping in memory for ultra-fast lookup without massive SQL JOINs
$master_ct_map = [];
$res_ct = $conn->query("SELECT kode, part_name, part_number, proses_name, ct_pcs, ct_jam, IFNULL(cavity, 1) as cavity FROM master_ct");
if($res_ct) while($ct = $res_ct->fetch_assoc()) $master_ct_map[$ct['kode']] = $ct;

// Operator history data will be built during log processing below
$opHistoryData = [];
...
// Pre-parse jam aktif bounds untuk fast lookup tanpa explode berulang-ulang
$parsedJamAktif = [];
foreach ($jamAktif as $jam) {
    $p = explode('-', $jam);
    if (count($p) === 2) {
...
// 1. QUERY RAW DATA DARI HISTORY_QUALITY (OPTIMIZED NO JOIN, ORDER BY timestamp)
$sql_logs = "SELECT hq.timestamp, hq.prodCount, hq.op_NIK, hq.kode_proses 
             FROM history_quality hq 
             WHERE hq.mcID = '$mcID' AND $filter_waktu_hq 
             ORDER BY hq.timestamp ASC";
...
if ($res_logs && $res_logs->num_rows > 0) {
    while($row = $res_logs->fetch_assoc()) {
        $logTime = substr($row['timestamp'], 11, 8);
        
        $curr_prodCount = (int)$row['prodCount'];
        if ($curr_prodCount < 0) continue; // Abaikan glitch/noise angka negatif

        $ct_info = $master_ct_map[$row['kode_proses']] ?? [];
        $cavity = (int)($ct_info['cavity'] ?? 1);
        if ($cavity < 1) $cavity = 1;
        $max_delta = 50 * $cavity; // Kapasitas stroke mesin per interval log dikalikan cavity
        ...
        } else if ($delta < 0 && $curr_prodCount <= $max_delta) {
            // ESP32/Mega Ter-Reset murni dari 0
            $qty_added = $curr_prodCount; 
            $prev_prodCount = $curr_prodCount;
        } else if ($delta == 0) {
            // Idle / standby
        } else {
            // Lonjakan anomali / Noise spike (> max_delta)
            // Abaikan penambahan ($qty_added = 0) dan JANGAN ubah $prev_prodCount
            continue;
        }
        ...
        if ($qty_added <= 0) continue; // Hemat CPU: skip bucketing jika tidak ada produksi

        $true_total_prod += $qty_added;

        // Fast Bucketing
        $matched_jam = null;
        foreach($parsedJamAktif as $jam => $b) {
            if ($b['s'] <= $b['e']) {
                if ($logTime >= $b['s'] && $logTime <= $b['e']) { $matched_jam = $jam; break; }
            } else {
                if ($logTime >= $b['s'] || $logTime <= $b['e']) { $matched_jam = $jam; break; }
            }
        }
        ...
        if (!$matched_jam) {
            ...
            if (!in_array($matched_jam, $jamAktif)) {
                ...
                $parsedJamAktif[$matched_jam] = ['s' => $start.':00', 'e' => $end.':00'];
                $hourlyActualSum[$matched_jam] = 0;
                $hourlyTargetSum[$matched_jam] = 0;
            }
        }
        
        $pn = !empty($ct_info['part_number']) ? $ct_info['part_number'] : '-'; 
        $pName = !empty($ct_info['part_name']) ? $ct_info['part_name'] : 'Unknown'; 
        $proses = !empty($ct_info['proses_name']) ? $ct_info['proses_name'] : 'Proses 1';
        $ct = !empty($ct_info['ct_pcs']) ? $ct_info['ct_pcs'] : 0; 
        $target = !empty($ct_info['ct_jam']) ? round($ct_info['ct_jam']) : 0; 
...
// Ambil jam acuan awal shift untuk sorting kronologis
$shift_base_minutes = 0;
if (!empty($jamAktif)) {
    $first_range = $jamAktif[0];
    $p = explode('-', $first_range);
    $startTime = trim($p[0]);
    $tParts = explode(':', $startTime);
    $shift_base_minutes = ((int)($tParts[0] ?? 0)) * 60 + ((int)($tParts[1] ?? 0));
}

$getTimeOffset = function($rangeStr) use ($shift_base_minutes) {
    $p = explode('-', $rangeStr);
    $startTime = trim($p[0]);
    $tParts = explode(':', $startTime);
    $m = ((int)($tParts[0] ?? 0)) * 60 + ((int)($tParts[1] ?? 0));
    return ($m - $shift_base_minutes + 1440) % 1440;
};

// Urutkan jam secara kronologis maju dari jam mulai shift
$jamAktifFinal = array_keys($hourlyActualSum);
usort($jamAktifFinal, function($a, $b) use ($getTimeOffset) {
    return $getTimeOffset($a) <=> $getTimeOffset($b);
});

// Re-index hourlyActualSum dan hourlyTargetSum agar urutan Chart.js sinkron dengan header tabel
$newHourlyActual = [];
$newHourlyTarget = [];
foreach ($jamAktifFinal as $jam) {
    $newHourlyActual[$jam] = $hourlyActualSum[$jam] ?? 0;
    $newHourlyTarget[$jam] = $hourlyTargetSum[$jam] ?? 0;
}
$hourlyActualSum = $newHourlyActual;
$hourlyTargetSum = $newHourlyTarget;
